...
export async function optimiseLogo(file:File):Promise<string>{
 ...
 return await new Promise<string>(resolve=>{const image=new Image();image.onload=()=>{const limit=512,scale=Math.min(1,limit/Math.max(image.width,image.height)),canvas=document.createElement("canvas");canvas.width=Math.max(1,Math.round(image.width*scale));canvas.height=Math.max(1,Math.round(image.height*scale));const context=canvas.getContext("2d");if(!context){resolve(source);return}context.drawImage(image,0,0,canvas.width,canvas.height);resolve(canvas.toDataURL("image/webp",.86))};image.onerror=()=>resolve(source);image.src=source});
}
